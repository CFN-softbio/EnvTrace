...
## Worked Example from the EnvTrace Paper
A demonstration of the EnvTrace method (written before the creation of the package) at a synchrotron beamline is available at [`backend/tests/test_op_cog.py`](https://github.com/CFN-softbio/VISION/blob/main/backend/tests/test_op_cog.py) in the [VISION](https://github.com/CFN-softbio/VISION) project.

This file contains a proof of concept semantic code scoring algorithm for the program synthesis part of the Synchrotron AI assistant (VISION), including a beamline simulator, this allows tracking PV changes to generate a more accurate coding score.

This EnvTrace package was based on the ideas implemented in the VISION evaluation script, to make a flexible package for people to compare code when they provide execution traces.

### Dataset and Experiments
The experiments generated by evaluating LLMs on the VISION datasets ([`backend/tests/datasets/op_cog_dataset.json`](https://github.com/CFN-softbio/VISION/blob/main/backend/tests/datasets/op_cog_dataset.json) and [`backend/tests/datasets/archived/original_op_cog_dataset.json`](https://github.com/CFN-softbio/VISION/blob/main/backend/tests/datasets/archived/original_op_cog_dataset.json)) using EnvTrace are uploaded on https://zenodo.org/records/17526264 (10.5281/zenodo.17526264).

These results are analyzed and described in the paper ... (pending release).
